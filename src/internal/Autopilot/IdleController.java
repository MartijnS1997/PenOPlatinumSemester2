 ...
 * A controller for controlling the drone while it is idle (and up in the air)
 */
public class IdleController extends TurnBasedController{

    public IdleController(AutoPilot autopilot) {
        super(autopilot);
    }
    ...
    public AutopilotOutputs getControlActions(AutopilotInputs_v2 currentInputs, AutopilotInputs_v2 previousInputs) {
        TurnBasedController.TurnControl turnControl = this.getTurnControl();
    ...
    /**
     * Checks if the drone was assigned a new delivery
     * if it has, return true, if not, false
     * @return true if and only if the current delivery request sent to the communicator by the overseer is not null
     */
    private boolean hasNewDelivery(){
        ...
        //check if there is a new delivery:
     ...
     */
    public void configureIdleController(AutopilotInputs_v2 currentInputs, float cruisingAltitude){
    ...
    private void configureTurnController(){
        ...
        TurnBasedController.TurnControl turnControl = new TurnBasedController.TurnControl(turn, turnPhysX, standardOutputs, cruisingAltitude);
     ...
     */
    private TurnBasedController.TurnControl getTurnControl() {
        return turnControl;
    }
     ...
     */
    private void setTurnControl(TurnBasedController.TurnControl turnControl) {
     ...
     * @param assignedPackage the lock status to set
     */
    private void setAssignedPackage(boolean assignedPackage) {
     ...
     */
    private TurnBasedController.TurnControl turnControl;
     ...
     */
    private boolean assignedPackage = false;
