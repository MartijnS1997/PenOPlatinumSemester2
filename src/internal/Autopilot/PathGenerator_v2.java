...
public class PathGenerator_v2{
	...
	public AutopilotTurn getNextTurn(){
		//get the number of turns that were already requested
		List<AutopilotTurn> turns = this.getTurnList();
		int nbReqTurns = this.getTurnsRequested();
		int nbOfTurnsAvailable = turns.size();

		if(nbOfTurnsAvailable == nbReqTurns){
			return null;
		}

		AutopilotTurn nextTurn =  turns.get(nbReqTurns);
		incrementTurnsRequested();
		return nextTurn;
	}
	...
	/**
	 * Getter for the number of turns that were already requested by the getNextTurn method
	 * if the counter hits the number of turns in the turn list we retrun a null to signal the end of the
	 * path has been reached
	 * @return number of turns that were already requested
	 */
	private int getTurnsRequested() {
		return turnsRequested;
	}

	/**
	 * Increments the number of times that the getNextTurn method was invoked
	 */
	private void incrementTurnsRequested() {
		this.turnsRequested++;
	}
	 ...
	 */
	private List<AutopilotTurn> turnList = new ArrayList<>();

	/**
	 * The counter that keeps track of the number of turns that were requested by the user of the generator
	 * if the counter reaches the size of the turn list, we don't have any turns to provide anymore
	 */
	private int turnsRequested = 0;
