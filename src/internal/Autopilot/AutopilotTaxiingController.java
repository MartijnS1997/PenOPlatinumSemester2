 ...
 *
 * //TODO implement the state machine & expand the conditions for finishing and initiating the controllers
 * //TODO rewrite the target function for the to runway taxiing
 */
...
//    }
//
//    private TaxiingState getNextActiveState(AutopilotInputs_v2 currentInputs, AutopilotInputs_v2 previousInputs) {
//
//    }
//
...
//
//    /**
//     * Adjusts the control outputs such that the drone stays within the airport borders
//     *
//     * @param airportBorders the borders of the airport
//     * @param outputs        the outputs to write the correction to
//     * @param inputs         the inputs most recently received from the testbed
//     */
...
//
//        //approx the velocity
//        Vector approxVelocity = getVelocityApprox(currentInputs, prevInputs);
...
////            if (targetGateID == 0) taxiTarget = getAutopilot().getCommunicator().getAirportByID(targetAirportID).getGateZeroPosition();
////            if (targetGateID == 1) taxiTarget = getAutopilot().getCommunicator().getAirportByID(targetAirportID).getGateOnePosition();
////
...
//     */
//    private void brakeTurningControls(ControlOutputs outputs, AutopilotInputs_v2 currentInputs, AutopilotInputs_v2 prevInputs) {
...
//
//    /**
//     * Getter for the airport that the drone is currently standing on
//     * @return the airport data of the airport the drone is currently located at
//     */
...
//
//    /**
//     * Setter for the airport on which the drone is currently located
//     * @param airport the airport at which the drone is currently located
//     */
...
//
//    /**
//     * The airport the drone is currently standing on, this info is used to get the controls right
//     */
...
//
//    /**
//     * A private interface to describe what a sub controller of the taxiing controller needs to do
//     */
//    private interface TaxiController{
//        boolean hasReachedTaxiObjective(AutopilotInputs_v2 currentInputs, AutopilotInputs_v2 previousInputs);
//
//        ControlOutputs getControlActions(AutopilotInputs_v2 currentInputs, AutopilotInputs_v2 previousInputs);
//    }
//
//    /**
//     * The controller used to reach the target set by the autopilot
//     */
//    private MoveToTargetController moveToTargetController;
//
//    /**
//     * The controller to set the orientation right for the drone
//     */
//    private OrientationController orientationController;
//
//    /**
//     * The controller to get the drone to a standstill
//     */
//    private FullBrakeController fullBrakeController;
//
//    private class MoveToTargetController implements TaxiController{
//
//        /**
//         * Default constructor
//         */
//        public MoveToTargetController() {
//
//        }
//
//        @Override
//        public boolean hasReachedTaxiObjective(AutopilotInputs_v2 currentInputs, AutopilotInputs_v2 previousInputs) {
...
//            //to the target
//            Vector target = AutopilotTaxiingController.this.getTarget();
//            //get the current pos
//            Vector position = Controller.extractPosition(inputs);
//
...
//            ControlOutputs outputs = new ControlOutputs(getStandardOutputs());
//            brakeTurningControls(outputs, currentInputs, previousInputs);
...
//
//    /**
//     * Constroller to set the orientation of the drone right if the situation requires
//     */
//    private class OrientationController implements TaxiController{
//
//        @Override
//        public boolean hasReachedTaxiObjective(AutopilotInputs_v2 currentInputs, AutopilotInputs_v2 previousInputs) {
//
//            return checkHasFinishedTurn(currentInputs);
//        }
//
//        /**
//         * Checks if the drone has finished its initial turn
//         */
//        private boolean checkHasFinishedTurn(AutopilotInputs_v2 currentInputs) {
//            //TODO modify such that it only turns to the orientation of the runway
//            Vector target = AutopilotTaxiingController.this.getTarget();
//            //the angle between the heading vector of the drone and the target is small enough, we may go to the next state
//            //the full brake state
//            float angle = angleToTarget(target, currentInputs);
...
//
//            Vector target = AutopilotTaxiingController.this.getTarget();
//            float maxBrake = AutopilotTaxiingController.this.getConfig().getRMax();
//            float maxThrust = AutopilotTaxiingController.this.getConfig().getMaxThrust();
//
//            //calculate the angle between the heading vector and the target vector
//            float angle = angleToTarget(target, currentInputs);
//
...
//
//        /**
//         * Getter for the target used to turn the drone to, to get the orientation right (we want the drone to align
//         * with the heading vector of the runway it is currently facing along with)
//         * @param currentInputs the inputs most recently received from the testbed
//         * @return the target calculated for the drone
//         */
//        private Vector calcTarget(AutopilotInputs_v2 currentInputs){
//            //get the current ground location
//            Vector dronePosGround = extractGroundPosition(currentInputs);
//            //get the heading vector of the drone in heading axis system & get the orientation
//            Vector headingDroneHa = new Vector(0,0,-1);
//            Vector orientation = extractOrientation(currentInputs);
//            //get the heading vector in the world axis system (relative to the drone position)
//            Vector droneHeadingWorldRel = PhysXEngine.headingOnWorld(headingDroneHa, orientation);
//            //get the heading vector of the airport the drone is currently at
//            MapAirport airport = AutopilotTaxiingController.this.getAirport();
//            Vector airportHeading = airport.getHeadingVector();
//            //check if we're pointing in the same direction
//            float scalarProd = airportHeading.scalarProduct(droneHeadingWorldRel);
//            //turn the direction of the airport heading according to the scalar product, if it was positive the
//            //directions are the same, if not, the direction of the airport heading must be changed
//            Vector targetHeading = airportHeading.scalarMult(signum(scalarProd));
//            //now scale the target heading to the lookahead distance to get relative reference point
//            Vector referenceRel = targetHeading.scalarMult(getLookaheadDistance());
//
//            //now sum the target heading with the position of the drone (at ground level)
//            return dronePosGround.vectorSum(referenceRel);
//        }
//
//        /**
//         * Getter for the lookahead distance used to determine a reference for the orientation controller
//         * this is the relative distance between the target and the position of the drone
...
//
//        /**
//         * The lookahead distance, used to place a reference for the orientation
//         */
//        private final static float lookaheadDistance = 100;
//
//    }
//
//    /**
//     * An implementation of a controller that brakes to the full extent
//     */
//    private class FullBrakeController implements TaxiController{
//
//        @Override
//        public boolean hasReachedTaxiObjective(AutopilotInputs_v2 currentInputs, AutopilotInputs_v2 previousInputs) {
//            //TODO also cope with the case that the drone needs to set its orientation right
//            //calculate an approx for the velocity
//            float velocityApprox = getVelocityApprox(currentInputs, previousInputs).getSize();
//            float minVelocity = AutopilotTaxiingController.STANDSTILL_VELOCITY;
//            return velocityApprox < minVelocity;
//        }
//
//        /**
//         * Getter for the control actions of the drone to brake of the full extent
//         * @param currentInputs the current inputs
//         * @param previousInputs the previous inputs
//         * @return the control outputs needed to get the drone to a standstill
//         */
//        @Override
//        public ControlOutputs getControlActions(AutopilotInputs_v2 currentInputs, AutopilotInputs_v2 previousInputs) {
//            ControlOutputs outputs = new ControlOutputs(getStandardOutputs());
